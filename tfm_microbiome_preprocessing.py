# -*- coding: utf-8 -*-
"""TFM_Microbiome_preprocess_DiseaseSpecific.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ERgUoiLz5fZLlv4VDcye3MbM04djgfA9
"""
...
import import_ipynb
import TFM_featureselection

"""**Leemos los datos de los ficheros csv/tsv:**
1. metadata, 
2. metabolomics (mtb)
3. metagenomics (mgx) 
4. metatranscriptomics (mtx)
5. IDs participantes seleccionados (3omics + 4tps)
"""
...
MTX = TFM_featureselection.pheno_to_numerical(MTX)
# prepare input (RNA abundances) and output (diagnosis) data 
MTX_X = MTX.iloc[:,7:len(MTX.columns)]
MTX_y = MTX.iloc[:,2]
MTX_X = TFM_featureselection.fill_NA(MTX_X)
# FEATURE SELECTION: Removing features with low variance
FS_MTX = TFM_featureselection.variance_threshold_selector(MTX_X)
...
mtx_top_chi2, mtx_test = TFM_featureselection.topfeatures_chi2(X_train, y_train, X_test)
...
mtx_filtered = TFM_featureselection.get_fs_columns(mtx_top_chi2, mtx)
...
# CAMBIAR UC CD Y NONIBD POR NUMEROS
MGX = TFM_featureselection.pheno_to_numerical(MGX)
# prepare input (RNA abundances) and output (diagnosis) data 
MGX_X = MGX.iloc[:,7:len(MGX.columns)]
MGX_y = MGX.iloc[:,2]
MGX_X = TFM_featureselection.fill_NA(MGX_X)
# FEATURE SELECTION: Removing features with low variance
FS_MGX = TFM_featureselection.variance_threshold_selector(MGX_X)
# SPLIT TRAIN AND TEST DATASET
X_train, X_test, y_train, y_test = train_test_split(FS_MGX, MGX_y, stratify=MGX_y, test_size=0.33)
...
MTB = TFM_featureselection.pheno_to_numerical(MTB)
# prepare input (RNA abundances) and output (diagnosis) data 
MTB_X = MTB.iloc[:,7:len(MTB.columns)]
MTB_y = MTB.iloc[:,2]
#fill NA's
MTB = TFM_featureselection.fill_NA(MTB_X)
# FEATURE SELECTION: Removing features with low variance
FS_MTB = TFM_featureselection.variance_threshold_selector(MTB_X)
# SPLIT TRAIN AND TEST DATASET
X_train, X_test, y_train, y_test = train_test_split(FS_MTB, MTB_y, stratify=MTB_y, test_size=0.33)
...
# Feature Selection for Output
#Extract 200 best features by a chi-squared test
...
cm_RF, RF_clf, y_prob = TFM_featureselection.random_forest_clf(X_train, y_train, X_test, y_test)
...
rA = pd.merge(metadata, mtb, left_index=True, right_index =True, sort=False, how='outer')
rB=pd.merge(rA, mgx, left_index=True, right_index =True, sort=False, how='outer' )
rf = pd.merge(rB, mtx, left_index=True, right_index =True, sort=False, how='outer')
rf = rf.fillna(0)
...
rf_top_chi2, rf_test = TFM_featureselection.topfeatures_chi2(X_train, y_train, X_test)
